...
Other completed modules not on original roadmap: =init-recentf.el=, =init-dired.el=, =init-web.el=, =init-highlight-symbol.el=, =init-rainbow-delimiters.el=, =init-hydra.el=, =init-ace-window.el=, =init-avy.el=, =init-mwim.el=, =init-undo-tree.el=, =init-treemacs.el=, =init-git.el=, =init-multiple-cursors.el=.
...
4. Start Emacs — packages install automatically when modules need them

5. Add feature modules in =modules/= as needed

6. Set up Org directory:
   #+BEGIN_SRC bash
   mkdir -p ~/org
   #+END_SRC
