...
Other completed modules not on original roadmap: =init-recentf.el=, =init-dired.el=, =init-web.el=, =init-highlight-symbol.el=, =init-rainbow-delimiters.el=.
...
3. =C-x w r= — Resize panel (h/j/k/l to adjust, q to quit)
4. =C-x b b= — Switch buffer (Consult: includes recentf + bookmarks)
