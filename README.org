...
├── init.el                  # Entry point (load core → splash → modules → Enter/file)
...
early-init.el → init.el → splash → core/*.el → modules/*.el → Press Enter → dired

# File mode (emacs file.txt / emacs -nw file.txt)
early-init.el → init.el → splash → core/*.el → modules/*.el → open file (no wait)
#+END_SRC
...
6. =emacs-startup-hook= — Restore GC to 16MB, show summary, detect startup mode:
...
| =C-j m=   | (hydra-multiple-cursors) | Multi-cursor panel |
...
Provided by =init-multiple-cursors.el=. Multi-cursor editing with a hydra operation panel.

| Key              | Function                       | Description                    |
|------------------+--------------------------------+--------------------------------|
| =C-j m=          | =hydra-multiple-cursors/body=  | Open multiple cursors panel    |
| =C-S-<mouse-1>= | =mc/toggle-cursor-on-click=    | Toggle cursor at click point   |

In the multiple cursors panel (=C-j m=):

*** Cursor Navigation

| Key   | Function                       | Description                     |
|-------+--------------------------------+---------------------------------|
| =n=   | =mc/mark-next-like-this=       | Mark next occurrence            |
| =p=   | =mc/mark-previous-like-this=   | Mark previous occurrence        |
| =N=   | =mc/skip-to-next-like-this=    | Skip current, mark next         |
| =P=   | =mc/skip-to-previous-like-this= | Skip current, mark previous    |
| =M-n= | =mc/unmark-next-like-this=     | Unmark last next                |
| =M-p= | =mc/unmark-previous-like-this= | Unmark last previous            |

*** Batch Operations

| Key | Function                     | Description                            |
|-----+------------------------------+----------------------------------------|
| =l= | =mc/edit-lines=              | Place cursor on each selected line     |
| =a= | =mc/mark-all-like-this=      | Mark all occurrences in buffer         |
| =s= | =mc/mark-all-in-region-regexp= | Mark by regexp in region             |
| =\vert= | =mc/vertical-align=      | Align cursors by input character       |
| =0= | =mc/insert-numbers=          | Insert incremental numbers             |
| =A= | =mc/insert-letters=          | Insert incremental letters             |
| =q= | quit                         | Exit panel                             |
...
6. On =emacs-startup-hook=: restore GC, show summary, detect startup mode
...
Multi-cursor editing via [[https://github.com/magnars/multiple-cursors.el][multiple-cursors]], with a hydra operation panel for visual command selection. Depends on =hydra= (=:after hydra=).
...
1. Select a word or region, then press =C-j m= to open the hydra panel
2. Use =n= / =p= to incrementally add cursors on matching occurrences
3. Use =N= / =P= to skip unwanted matches
4. Use =a= to mark all matches at once
5. Use =l= to place a cursor on each line of a selected region
6. Type normally — all cursors edit simultaneously
7. Press =C-g= to exit multi-cursor mode, or =q= to close the panel
...
Remove ="init-tramp"= from =skemacs-disabled-modules= in =init.el=.
...
|----------+-----------------------+-------------------------------------------------------+--------|
|        1 | =init-theme.el=       | Theme setup (modus + doom + ef + catppuccin)           | ✓ Done |
...
|        4 | =init-ace-window.el=  | Ace-window (window quick-jump)                        | ✓ Done |
|          | =init-avy.el=         | Avy (char/word/line jump)                             | ✓ Done |
|          | =init-mwim.el=        | Smart line beginning/end (mwim)                       | ✓ Done |
|          | =init-undo-tree.el=   | Undo-tree (tree-based undo/redo)                      | ✓ Done |
|        5 | =init-multiple-cursors.el= | Multiple-cursors with hydra panel                | ✓ Done |
|        6 | =init-project.el=     | Project management (built-in project.el)              | ✓ Done |
|        7 | =init-git.el=         | Magit + diff-hl                                       | ✓ Done |
|        8 | =init-lsp.el=         | lsp-bridge + Company + YASnippet                      | ✓ Done |
|        9 | =init-vterm.el=       | Vterm                                                 | ✓ Done |
|       10 | =init-treemacs.el=    | Treemacs file tree sidebar                            | ✓ Done |
|          | =init-ui.el=          | Modeline + other UI enhancements                      | —      |
|       11 | =init-flycheck.el=    | Syntax checking                                       | —      |
|       12 | =init-org.el=         | Org-mode full config                                  | —      |
|       13 | =init-agent-shell.el= | AI Agent (Claude Code + Cursor + Gemini via ACP)      | ✓ Done |
|       14 | =init-claude-code-ide.el= | Claude Code IDE (MCP bidirectional bridge)        | ✓ Done |
|       15 | =init-tramp.el=       | TRAMP remote file access (SSH)                        | ✓ Done (disabled) |

Other completed modules not on original roadmap: =init-recentf.el=, =init-dired.el=, =init-web.el=, =init-highlight-symbol.el=, =init-rainbow-delimiters.el=, =init-hydra.el=, =init-ace-window.el=, =init-avy.el=, =init-mwim.el=, =init-undo-tree.el=, =init-treemacs.el=, =init-git.el=, =init-multiple-cursors.el=, =init-agent-shell.el=, =init-readonly.el=, =init-tramp.el=.
...
1. =C-j m= — Open multiple cursors panel
2. Select word → =n= / =p= — Add cursors on next/previous match
3. Select word → =a= — Mark all same occurrences
4. Select lines → =l= — One cursor per line
5. =C-S-click= — Toggle cursor at mouse position
